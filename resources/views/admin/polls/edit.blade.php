...
<div class="max-w-2xl mx-auto">
    <a href="{{ route('admin.polls.show', $poll) }}" class="text-indigo-600 text-sm hover:underline mb-4 inline-block">← Back to Poll</a>

    <div class="bg-white rounded-xl border p-6">
        <h1 class="text-xl font-bold text-gray-800 mb-4">Edit Poll</h1>
        ...
        <form action="{{ route('admin.polls.update', $poll) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm @error('description') border-red-400 @enderror">{{ old('description', $poll->description) }}</textarea>
                @error('description')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>

            {{-- Dynamic Options with Alpine.js --}}
            @php
                $existingOptions = old('options', $poll->options->pluck('option_text')->toArray());
            @endphp
            <div x-data="{
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="ends_at"
                        value="{{ old('ends_at', $poll->ends_at?->format('Y-m-d')) }}"
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.polls.show', $poll) }}" class="text-gray-500 text-sm py-2 hover:text-gray-700">Cancel</a>
            </div>
        </form>
    </div>
</div>
