@extends('layouts.app')
@section('title', 'Edit Patrol Assignment')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('security.patrols.show', $patrol) }}" class="text-gray-400 hover:text-gray-600 transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <h1 class="text-lg font-semibold text-gray-800">Edit Patrol Assignment</h1>
    </div>
    ...
    <form action="{{ route('security.patrols.update', $patrol) }}" method="POST"
        ...
        <div>
            ...
            <input type="text" id="area" name="area" value="{{ old('area', $patrol->area) }}"
        ...
        <div>
            ...
            <select id="assigned_to" name="assigned_to"
                ...
                <option value="{{ $officer->id }}"
                    {{ old('assigned_to', $patrol->assigned_to) == $officer->id ? 'selected' : '' }}>
                    {{ $officer->name }}
                </option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="datetime-local" id="shift_start" name="shift_start"
            ...
            <div>
                ...
                <input type="datetime-local" id="shift_end" name="shift_end"
        ...
        <div>
            ...
            <select id="status" name="status"
                ...
                <option value="{{ $val }}" {{ old('status', $patrol->status) === $val ? 'selected' : '' }}>
                    {{ $label }}
                </option>
                @endforeach
            </select>
